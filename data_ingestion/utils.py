...
def format_table_of_contents(text_lines: list[str]) -> list[list]:
    ...
    for line in text_lines:
        stripped = line.replace('|', '').strip() # remove |, VLM often put in table form
        # if empty line -> skip
        if stripped == '':
            continue
            ...
            if extract_s_p and len(extract_s_p[0]) == 2:
                ...
                toc.append(
                    [
                        top_level 
                        + int(has_bold) 
                        + (extracted_section_name.count('.') # h_n becomes h_n+1 is still acceptable
                        if begin_with_number.match(extracted_section_name) # if annex we dont count dot
                        else 0), 
...
def fix_page_header_using_toc(text: str, toc, metadata_page_num) -> str:
    ...
    d_motif = {
        pattern: level for level, pattern, _ in concerned_page
    }
    res = []
    for l in text.splitlines():
        # WHO guidelines has mark in the headlines ...
        if '<mark>' in text:
            l = re.sub(r'^(#+)\s+.*?<mark>(.*?)</mark>\s*', r'\1 \2 ', l, flags=re.MULTILINE).strip()
...
def fix_doc_header_using_toc(toc: list[tuple], markdown_text, docs, splitter):
    ...
    for page_md, page_pdf in zip(markdown_text, docs):
        metadata_page_num = page_pdf.metadata['page_label'] \
        ...
        page_md_text = fix_page_header_using_toc(page_md["text"], toc, metadata_page_num)
